...
class Settings:
    ...
    # Base URL Agora's Conversational AI Engine itself can reach to call
    # our MCP tool server (see app/mcp/tool_server.py). This must be a
    # publicly reachable URL -- Agora's servers, not your phone, make
    # this request -- so on local dev it needs the same tunnel used for
    # the Android backend itself (see docs/local-tunnels.md). Empty
    # string means "don't attach an MCP server" (agent falls back to
    # plain conversation, no tool-calling).
    mcp_tool_server_public_url: str = ""
    enable_agent_tool_calling: bool = True
    ...
    def from_env(cls) -> "Settings":
        ...
        return cls(
            ...
            mcp_tool_server_public_url=os.getenv("MCP_TOOL_SERVER_PUBLIC_URL", "").strip(),
            enable_agent_tool_calling=os.getenv("ENABLE_AGENT_TOOL_CALLING", "true").strip().lower()
            not in ("false", "0", "no"),
        )
